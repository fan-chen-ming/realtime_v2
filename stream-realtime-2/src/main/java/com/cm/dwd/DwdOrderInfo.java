...
/**
 * @Package com.cm.dwd.DwdOrderInfo
 * @Author chen.ming
...
public class DwdOrderInfo {
    public static void main(String[] args) throws Exception {
        ...
        KafkaSource<String> kafkaSourceBd = FlinkSourceUtil.getKafkaSource("chenming_db", "dwd_app");
        ...
        KeyedStream<JSONObject, String> oidBy = operator.keyBy(data -> data.getString("order_id"));

        SingleOutputStreamOperator<JSONObject> process = idBy.intervalJoin(oidBy).between(Time.minutes(-2), Time.minutes(2)).process(new ProcessJoinFunction<JSONObject, JSONObject, JSONObject>() {
            @Override
